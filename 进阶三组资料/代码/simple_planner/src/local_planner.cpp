...
namespace robomaster{
    ...
    public:
        LocalPlanner(ros::NodeHandle& given_nh):nh(given_nh),plan_(false), prune_index_(0){
            ...
            nh.param<int>("plan_frequency", plan_freq_, 50); //规划频率 50hz
            ...
            // PIDController controller_x(1.0,0,0.01);
            // PIDController controller_y(1.0,0,0.01);
    ...
    private:
        ...
        void Plan(const ros::TimerEvent& event){
            ...
            if (plan_){
                ...
                //tmp_pose is base on odom frame
        ...
        void FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
            ...
            // calculate velocity
            //diff_yaw=diff_yaw+(diff_yaw>0?2:-2);
            if( diff_yaw > 0 ){
            ...
            //当偏航角差 diff_yaw 较大时，实体的速度会降低，当偏航角差 diff_yaw 较小时，实体的速度会接近最大速度 max_speed_。
            //这样做的目的可能是为了在转弯时减小速度，以提高机器人的稳定性和安全性
