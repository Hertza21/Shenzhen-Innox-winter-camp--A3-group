...
namespace robomaster
{
  ...
  private:    
    ...
    {
      //be coded by ZhangTao
      //获取机器人当前位置
      geometry_msgs::PoseStamped current_pose;
      nav_msgs::Path book;

      book.header.frame_id = global_frame_;
      

      GetGlobalRobotPose(tf_listener_,global_frame_,current_pose);

      //检测是否到达最后一个航点
      if(path_index_>= fixed_path.poses.size())
      {
          path_index_=0;
      }
      const geometry_msgs::PoseStamped &next_waypoint= fixed_path_.poses[path_index_++];
